...
void cg::renderer::dx12_renderer::init()
{
	...
	view_port = CD3DX12_VIEWPORT(0.f, 0.f, (float) settings->width, (float) settings->height);
...
void cg::renderer::dx12_renderer::initialize_device(ComPtr<IDXGIFactory4>& dxgi_factory)
{
	...
	hardware_adapter->GetDesc(&adapterDesc);
...
void cg::renderer::dx12_renderer::create_swap_chain(ComPtr<IDXGIFactory4>& dxgi_factory)
{
	...
	swapChainDesc.BufferDesc.Width = this->get_width();
	swapChainDesc.BufferDesc.Height = this->get_height();
	swapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	swapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	swapChainDesc.OutputWindow = cg::utils::window::get_hwnd();
	swapChainDesc.SampleDesc.Count = 1;
	swapChainDesc.Windowed = TRUE;

	ComPtr<IDXGISwapChain> swapChain;
	THROW_IF_FAILED(dxgi_factory->CreateSwapChain(
...
std::filesystem::path cg::renderer::dx12_renderer::get_shader_path(const std::string& shader_name)
{
	...
	auto shader_path = std::filesystem::path(buffer).parent_path().parent_path() / shader_name;
	return shader_path;
}
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	for (size_t i = 0; i < model->get_index_buffers().size(); i++) {
		...
		const UINT vertex_buffer_size = vertex_buffer_data->get_size_in_bytes();
		...
		const UINT index_buffer_size = index_buffer_data->get_size_in_bytes();
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	for (size_t s = 0; s < model->get_index_buffers().size(); s++) {
		...
		command_list->DrawIndexedInstanced((UINT) model->get_index_buffers()[s]->get_number_of_elements(), 1, 0, 0, 0);
	}
