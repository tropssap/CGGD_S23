...
namespace cg::renderer
{
	...
	inline void raytracer<VB, RT>::build_acceleration_structure()
	{
			...
			while(index_id < index_buffer->get_number_of_elements()) {
				...
				acceleration.add_triangle(triangle);
			}
			acceleration_structures.push_back(acceleration);
			}
	}
...
#pragma omp parallel for
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					float u = (2.f * (float) x + jitter.x) / (float) (width - 1) - 1.f;
					float v = (2.f * (float) y + jitter.y) / (float) (height - 1) - 1.f;
					...
					history_pixel += sqrt(p.color.to_float3() * frame_weight);
	...
	float2 raytracer<VB, RT>::get_jitter(int frame_id)
	{
			...
			while (index > 0) {
			result.x += (float)(index % base_x) * fraction;
			index /= base_x;
			fraction *= inv_base;
			}
			...
			fraction = inv_base;
			while (index > 0) {
			result.y += (float)(index % base_y) * fraction;
			index /= base_x;
			fraction *= inv_base;
			}
			return result - 0.5f;
