...
{
	theta = in_theta * (float) M_PI / 180.f;
}

void cg::world::camera::set_phi(float in_phi)
{
	phi = in_phi * (float) M_PI / 180.f;
}

void cg::world::camera::set_angle_of_view(float in_aov)
{
	angle_of_view = in_aov * (float) M_PI / 180.f;
}
...
const DirectX::XMMATRIX cg::world::camera::get_dxm_view_matrix() const
{
	...
	return  DirectX::XMMatrixLookToRH(eye_position, eye_direction, up);
}
...
{
	float f = 1.f / std::tanf(angle_of_view / 2);
	return float4x4{
